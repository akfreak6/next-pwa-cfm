...
const StudentMngmt: FC = () => {
  const [students, setStudents] = React.useState({ name: '', rollNumber: '', hobbies: [''], status: [''], id: null });
  ...
  //       hobbies: Object(students?.hobbies)
  //     });
  ...
  const addStudent = React.useCallback(async () => {
    if (
      students?.name &&
      students?.rollNumber &&
      students?.hobbies.some((hobby) => hobby !== '') && // Ensure hobbies are entered
      students?.status.length == 1 && // Ensure valid data
      ...
      try {
        ...
        setStudents({ name: '', rollNumber: '', hobbies: [''], status:[], id: null });
      } catch (error) {
  ...
  const updateStudent = React.useCallback(async () => {
    if (students?.id && students?.name && students?.rollNumber &&
      students?.hobbies.some((hobby) => hobby !== '') && // Ensure hobbies are entered
      students?.status.length > 0 && // Ensure valid data
      students?.status.some((skill) => skill !== '')) {
            ...
            studentList?.map((i: any, index: number) => {
              return (
                <tr key={index}>
                  ...
                  <td style={{ paddingTop: '10px' }} > <span style={{ paddingLeft: '10px', paddingRight: '10px' }}>{i.status}</span></td>
      ...
      <div style={{ paddingLeft: '30px' }} >
        ...
        <div >
          <div id="nameFields" style={{display: 'block'}}>
            <label htmlFor="Name" style={{ display:'block', paddingRight: '10px' }}>Name</label>
          ...
          <div id="rollFields" style={{display: 'block'}}>
            <label htmlFor="roll" style={{ display:'block', paddingRight: '10px' }}>Roll Number </label>
              ...
              {students.hobbies.map((hobby, index) => (
                <div key={index}>
                  <input
                    ...
                    value={hobby}
                    onChange={(e) =>
                      setStudents({ ...students, hobbies: [...students.hobbies.slice(0, index), e.target.value, ...students.hobbies.slice(index + 1)] })
                    }
                    placeholder={`Hobby ${index + 1}`}
                  />
                  {index !== 0 && (
                    <button onClick={() => setStudents({ ...students, hobbies: students.hobbies.filter((_, i) => i !== index) })}>
                      - Remove
                    </button>
                  )}
                </div>
              ))}
              <button onClick={() => setStudents({ ...students, hobbies: [...students.hobbies, ''] })}>
                + Add another hobby
              </button>
          </div>
          ...
          <MultiSelectDropdown
              ...
              selectedValues={students.status} // Access the first selection (if any)
              onValuesChange={(value) => setStudents({ ...students, status: value })} // Update state with a single value
            />
          </div>
          {
            students?.id ? (
